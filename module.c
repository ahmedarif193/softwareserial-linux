...
static void soft_uart_close(struct tty_struct* tty, struct file* file)
{
  ...
  int wait_time = 0;
  while ((raspberry_soft_uart_get_tx_queue_size(tty) > 0)
...
static int soft_uart_probe(struct platform_device *pdev)
{
  ...
  struct tty_struct *tty;
    ...
    struct tty_driver *soft_uart_driver = tty_alloc_driver(uart_instance_count, TTY_DRIVER_REAL_RAW | TTY_DRIVER_DYNAMIC_DEV);
